...
/*
    ...
    /*
    slidingSwipeDownRecognizer = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(slideDown)];
    slidingSwipeDownRecognizer.delegate = self;
    slidingSwipeDownRecognizer.direction = UISwipeGestureRecognizerDirectionUp;
	[self.slidingView addGestureRecognizer:slidingSwipeDownRecognizer];
    
    slidingSwipeUpRecognizer = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(slideUp)];
    slidingSwipeUpRecognizer.delegate = self;
    slidingSwipeUpRecognizer.direction = UISwipeGestureRecognizerDirectionDown;
    [self.slidingView addGestureRecognizer:slidingSwipeUpRecognizer];
    *//*
...
-(void)move:(id)sender {
    ...
    if (translatedPoint.y >= 480-SLIDING_VIEW_VISIBLE_HEIGHT + SLIDING_VIEW_HEIGHT) {
        translatedPoint.y = 480- SLIDING_VIEW_VISIBLE_HEIGHT + SLIDING_VIEW_HEIGHT;
    }
    ...
    if([(UIPanGestureRecognizer*)sender state] == UIGestureRecognizerStateEnded) {
        ...
        if(UIDeviceOrientationIsPortrait([[UIDevice currentDevice] orientation])) {
            ...
            else if(finalY >=  480-SLIDING_VIEW_VISIBLE_HEIGHT + SLIDING_VIEW_HEIGHT) {
                
                finalY =  480-SLIDING_VIEW_VISIBLE_HEIGHT + SLIDING_VIEW_HEIGHT;
            }
        }
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    ...
    else if (touch.view == slidingView && gestureRecognizer == slidingSwipeDownRecognizer) {
        return YES;
    }
